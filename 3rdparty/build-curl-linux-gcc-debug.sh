...
./configure  --prefix=$CURL_ROOT/../lib/$OS/$COMPILER/$BUILDTYPE/curl  --enable-debug --enable-static=yes --enable-shared=no --enable-http --with-out \
 --with-ssl=$OPENSSL_ROOT/../lib/$OS/$COMPILER/$BUILDTYPE/openssl --with-zlib=$ZLIB_ROOT/../lib/$OS/$COMPILER/$BUILDTYPE/zlib
